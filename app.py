...
import time
import os
import plotly.graph_objects as go# ==================== FORCE TRUE DARK MODE + ELITE THEME ====================
st.set_page_config(page_title="NBA Projector 2025", layout="wide", page_icon="fire")st.markdown("""<style>
...
""", unsafe_allow_html=True)# ==================== PASSCODE ====================
PASSCODE = "getmoney"
    ...
    with col2:
        if st.button("ENTER", use_container_width=True, type="primary"):
            if code == PASSCODE:
                ...
                st.error("Wrong")
    st.stop()st.markdown("<h1 class='title-main'>NEXT GAME PROJECTOR</h1>", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; font-size:30px; opacity:0.9; margin-bottom:60px; color:#aaa;'>Opponent-adjusted • Real-time lock meter • For sharps only</p>", unsafe_allow_html=True)CACHE_DIR = "nba_cache_v2"
os.makedirs(CACHE_DIR, exist_ok=True)# ==================== DEFENSIVE RATINGS ====================
OPP_DEF_RATINGS = {
...
df.to_parquet(cache)
return dfdef predict_next_game(df, stat, n_recent=15):
    if len(df) < 15:
        avg = round(df[stat].mean(), 1) if not df.empty else 0
        return avg, None, None, 40d = df.tail(80).copy()
...
return round(pred, 1), round(pred-ci, 1), round(pred+ci, 1), lock# ==================== UI ====================
...
with c3:
    if st.button("Clear Cache", use_container_width=True):
        st.cache_data.clear()
        st.success("Cache cleared")if search:
    ...
    if matches.empty:
        ...
        name = pick.upper()    with st.spinner("Training model..."):
        logs = get_logs(pid)
    ...
    if len(logs) < 15:
        ...
        for i, s in enumerate(stats):
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(size=14, line=dict(width=4, color='white'))
            ))

            # MASSIVE GLOWING STAR
            fig.add_trace(go.Scatter(
                x=[40], y=[proj],
                mode='markers+text',
                ...
                textfont=dict(size=32, color="black", family="Arial Black"),
                showlegend=False,
                hoverinfo='skip'
            ))
            ...
            fig.add_trace(go.Scatter(
                x=[40], y=[proj],
                mode='markers',
                marker=dict(size=130, color=color, opacity=0.25),
                showlegend=False,
                hoverinfo='skip'
            ))
        ...
        with st.expander("Last 15 Games"):
            ...
            st.dataframe(show[['DATE','OPP','MIN','PTS','REB','AST','STL','BLK']], use_container_width=True)st.caption("Private • Opponent-adjusted • Nuclear glow • 2025 Season • Winners only")
