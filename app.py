...
<style>
    ...
    .chart-title {font-size: 42px !important; font-weight: 900; text-align: center; margin: 70px 0 30px 0; color: white; text-shadow: 0 0 40px rgba(0,255,157,0.6);}
</style>
...
def predict_next_game(df, stat, n_recent=15, next_opp_def=None):
    if len(df) < 15:
        return round(df[stat].mean(),1) if not df.empty else 0, None, None, 40
...
if search:
    ...
    if matches.empty:
        ...
        if len(logs) < 15:
            ...
            for s in stats:
                proj, lo, hi, lock = predict_next_game(logs, s, n_recent, next_def.get(s))
            ...
            glow = ["#FF006E","#00D4AA","#FFD60A","#9D4EDD","#FF6B00"]

            for i,s in enumerate(stats):
                proj = results[i]["PROJ"]
                col = glow[i]
                fig.add_trace(go.Scatter(x=last40['game'], y=last40[s], mode='lines+markers', name=s,
                                        line=dict(color=col,width=9), marker=dict(size=14,line=dict(width=4,color='white'))))
                fig.add_trace(go.Scatter(x=[40], y=[proj], mode='markers+text',
                                        marker=dict(symbol='star-diamond',size=80,color=col,line=dict(width=10,color='white')),
                ...
                fig.add_trace(go.Scatter(x=[40], y=[proj], mode='markers',
                                        marker=dict(size=130,color=col,opacity=0.25), showlegend=False))
